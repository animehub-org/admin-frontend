...
// Assuming API_URL is defined locally or fetched differently.
// For now, using what's available or importing from Consts if needed.
// But BasePage usually handles API interactions via getFromApi/postToApi.
import { API_URL } from "../Consts.ts";


export interface BaseFormState<T> extends FormState<T> {
    loading: boolean;
    err: any;
    title: string | null;
}
...
> extends BasePage<P, S> {
    ...
    protected handleImageUpload = async (e: ChangeEvent<HTMLInputElement>, fieldKey: string) => {
        ...
        try {
            ...
            // Use postToAdminApi which handles token and API URL automatically
            // Return type is expected to be compatible with what we need
            // Adjust the generic types <any, FormData> as needed based on ResponseType
            const response = await this.postToAdminApi<any, FormData>(`/images/upload?folder=nominees`, formData, {
